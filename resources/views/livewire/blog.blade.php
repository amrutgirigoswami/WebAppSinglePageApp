                                        ...
                                        <article class="blog-post">
                                            ...
                                            <div class="pt-4">
                                                ...
                                                <h2 class="h4"><a class="text-black"
                                                        href="{{ route('blog.detail', $article->id) }}" wire:navigate>{{ $article->title }}</a>
                                                </h2>
                                                <p>{{ Str::limit($article->content, 100) }}</p>
                                                <a href="{{ route('blog.detail', $article->id) }}" wire:navigate class="text-primary fw-bold"
                ...
                <div class="col-lg-3">
                    ...
                    <div class="widget widget-categories">
                        ...
                        <ul class="list-unstyled widget-list">
                            <li><a href="{{ route('blog') }}" wire:navigate>All<small class="ml-auto"></small></a>
                            </li>
                            @if ($categories->isNotEmpty())
                                @foreach ($categories as $category)
                                    <li><a href="{{ route('blog') . '?categorySlug=' . $category->slug }}"
                                            wire:navigate>{{ $category->name }} <small class="ml-auto"></small></a>
                                    </li>
                                @endforeach
                            ...
                            @foreach ($latestArticles as $latestArticle)
                                <ul class="list-unstyled widget-list">
                                    <li class="d-flex widget-post align-items-center">
                                        <a class="text-black" href="{{ route('blog.detail', $latestArticle->id) }}" wire:navigate>
                                            <div class="widget-post-image flex-shrink-0 me-3">
                                                <img class="rounded" loading="lazy" decoding="async"
                                                    src="{{ Storage::url($latestArticle->image) }}" alt="Post Thumbnail">
                                            </div>
                                        </a>
                                        <div class="flex-grow-1">
                                            <h5 class="h6 mb-0"><a class="text-black"
                                                    href="{{ route('blog.detail', $latestArticle->id) }}">{{ $latestArticle->title }}</a></h5>
                                            <small>{{ $latestArticle->created_at->format('F j, Y') }}</small>
                                        </div>
                                    </li>
                                </ul>
                            @endforeach
